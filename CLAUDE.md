...
**Dates are in Qatar time.** The business runs in Qatar (`Asia/Qatar`, UTC+3) but entries are sometimes made from abroad, so every business date (`paid_at`, `withdrawn_at`, etc.) comes from `todayStr()` / `addDays()` / `periodRange()` in `helpers.js`, never from `new Date().toISOString()` or the device's local date. The exact entry moment is `created_at` (shown via `fmtTime`).
